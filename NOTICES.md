...
Glaze links or bundles the components below. Most are permissively
licensed and need only this acknowledgement; the two that carry real
obligations are called out first, because they are the ones a reader is
actually looking for.

This file is generated from `cargo metadata` for the Rust side and
maintained by hand for everything else. Regenerate the crate list after
changing dependencies.

---

## LAME — LGPL-2.0, statically linked

Glaze writes MP3 with **LAME 3.100** (<https://lame.sourceforge.io>), via
the `mp3lame-encoder` / `mp3lame-sys` crates, which vendor LAME's source
and compile it into the application binary. LAME itself is distributed
under the **GNU Library General Public License, version 2** (the
`COPYING` file shipped in `lame-3.100`); the wrapper crates declare
LGPL-3.0.

LAME is not modified. Because it is linked statically, the LGPL's
relinking provision applies: **you are entitled to the object files
needed to relink Glaze against a modified or different build of LAME.**

To request them, open an issue at
<https://github.com/jkbennemann/glaze-releases/issues> titled "LGPL
relink request". You will be sent the compiled objects for the release
you name, for both architectures.

You can also produce them yourself, which is faster: `mp3lame-sys`
vendors LAME 3.100 unmodified, so `cargo build --release` in the Glaze
source tree reproduces the same objects under
`target/<triple>/release/build/mp3lame-sys-*/out/`. Substituting your own
LAME there and rebuilding is the relinking the licence has in mind.

macOS ships no MP3 encoder of its own, which is why this dependency
exists at all; every other export format Glaze offers is produced by
Core Audio.

## Symphonia — MPL-2.0

Audio import (WAV, MP3, M4A, FLAC, OGG, AIFF, CAF) uses **Symphonia**
(<https://github.com/pdeljanov/Symphonia>), under the Mozilla Public
License 2.0. The MPL's copyleft is per-file and binds only Symphonia's
own files, which Glaze does not modify. Source for the exact versions
below is available from crates.io and from the project above.

The same applies to the MPL-2.0 crates Glaze inherits through Tauri:
`cssparser`, `cssparser-macros`, `dtoa-short`, `selectors`, `option-ext`.

## Apple frameworks

Encoding to M4A (AAC), FLAC and AIFF invokes `/usr/bin/afconvert`, a
component of macOS. It is executed, never linked, and is not
redistributed with Glaze.

---

## Still to inventory

The voice runtime is a separate download (PyInstaller, ~441 MB) with its
own dependency tree — MLX, transformers, huggingface_hub, soundfile and
their transitive dependencies. Those notices are **not** in this file
yet.
...
Every crate linked into the application, with the licence its authors
declared. Where several are listed with `OR`, Glaze relies on the
permissive option.
...
- mio 1.2.0 — MIT
- moxcms 0.7.10 — BSD-3-Clause OR Apache-2.0
- mp3lame-encoder 0.2.5 — LGPL-3.0
...
- sherpa-onnx 1.12.39 — Apache-2.0
- sherpa-onnx-sys 1.12.39 — Apache-2.0
